...
class DepthCache(object):
    ...
    def __init__(self, symbol, conv_type: Callable = float):
        ...
        :param symbol: 创建深度缓存的交易对符号
        :type symbol: string
        :param conv_type: 可选，表示价格和数量的类型，默认为 float。
        ...
        self.conv_type: Callable = conv_type  # 转换类型
        self._log = logging.getLogger(__name__)  # 日志记录器
    ...
    def add_bid(self, bid):
        ...
        :return: None
        """
        self._bids[bid[0]] = self.conv_type(bid[1])  # 将买单价格和数量转换并存储
        if bid[1] == "0.00000000":  # 如果数量为0，则删除该买单
    ...
    def add_ask(self, ask):
        ...
        :return: None
        """
        self._asks[ask[0]] = self.conv_type(ask[1])  # 将卖单价格和数量转换并存储
        if ask[1] == "0.00000000":  # 如果数量为0，则删除该卖单
            del self._asks[ask[0]]

    def get_bids(self):
        """获取当前的买单列表。

        :return: 返回买单列表，价格和数量均为 conv_type 类型
        """
