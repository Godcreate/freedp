...
class BinanceSocketManager:
    ...
    def miniticker_socket(self, update_time: int = 1000):
        """启动一个用于所有交易的 miniticker WebSocket。

        该接口未在官方 Binance API 文档中列出，但它提供了 Binance 上的
        交易页面右侧列的数据。

        :param update_time: 回调之间的时间（毫秒），必须为 1000 或更大
        :type update_time: int

        :returns: 如果成功，返回连接键字符串，否则返回 False

        消息格式示例：
        [
            {
                'e': '24hrMiniTicker',  # 事件类型
                'E': 1515906156273,     # 事件时间
                's': 'QTUMETH',         # 交易对
                'c': '0.03836900',      # 收盘价
                'o': '0.03953500',      # 开盘价
                'h': '0.04400000',      # 最高价
